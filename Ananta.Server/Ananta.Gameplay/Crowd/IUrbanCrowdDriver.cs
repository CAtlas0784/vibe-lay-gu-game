...
public interface IUrbanCrowdDriver
{
    ...
    Task DespawnCrowdPedestrianAsync(TcpSession session, ulong entityId);

    Task SendPedestrianMoveAsync(TcpSession session, ulong entityId, float x, float y, float z, float facing, byte moveId);
